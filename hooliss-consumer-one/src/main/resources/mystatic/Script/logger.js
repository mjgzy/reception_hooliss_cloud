...
bdShare._LogPool = bdShare._LogPool || [], bdShare.ApiPVLogger || function(e) {
	...
	function f() {
		...
		if (t > 0) {
			...
			var n = [],
				...
				s = null,
				f = [0, 0, 0, 0, 0, 0, 0, 0];
			for (var l = 0; l < t; l++) s = e[l], f[r[s.key]] = 1, s.api.linkid && (i = s.api.linkid, delete s.api.linkid), n.push(
				u(s.key, s.api));
	...
	n.guid = "$BAIDU$", n.$$ = window[n.guid] = window[n.guid] || {
			...
			try {
				d = m(), a == "GET" && (i && (e += (e.indexOf("?") >= 0 ? "&" : "?") + i, i = null), r.noCache && (e += (e.indexOf(
					"?") >= 0 ? "&" : "?") + "b" + +(new Date) + "=1")), o ? d.open(a, e, s, o, u) : d.open(a, e, s), s && (d.onreadystatechange =
					v), a == "POST" && d.setRequestHeader("Content-Type", f["Content-Type"] || "application/x-www-form-urlencoded");
		...
		function() {
			...
			n.browser.safari = /(\d+\.\d)?(?:\.\d)?\s+safari\/?(\d+\.\d+)?/i.test(e) && !/chrome/i.test(e) ? +(RegExp.$1 ||
				RegExp.$2) : undefined
		}(), n.browser.ie = n.ie = /msie (\d+\.\d+)/i.test(navigator.userAgent) ? document.documentMode || +RegExp.$1 :
		undefined, n.browser.opera = /opera(\/| )(\d+(\.\d+)?)(.+?(version\/(\d+(\.\d+)?)))?/i.test(navigator.userAgent) ? +
		(RegExp.$6 || RegExp.$2) : undefined, n.dom = n.dom || {},
		function() {
	...
	var r = {
		...
		h = function() {
			...
			setInterval(function() {
				...
				if (r == 0) return;
				i();
				var o = ["http://nsclick.baidu.com/v.gif?pid=307", "type=3075", "l=" + r, "t=" + n.scrollTop, "s=" + n.spendTime,
					"v=" + n.viewHeight, "f=" + u * a, "r=" + encodeURIComponent(document.referrer), "u=" + encodeURIComponent(
		...
		p = function() {
			var e = 0;
			return function() {
				if (bdShare.velocity && (!bdShare.velocity.mainJsLoaded || !bdShare.velocity.cssLoadEnd) && e++ < 3) setTimeout(p,
					1e3);
				else {
					if (bdShare.velocity) {
